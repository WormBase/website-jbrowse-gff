...
parallel -j 2 $MAKEPATH --conf $CONFPATH  --species {} 2>1 | grep -v "Deep recursion"; mv 1 $LOGFILE ::: "${SPECIESLIST[@]}"
...
parallel -j 2 $UPLOADTOS3PATH --bucket $AWSBUCKET --local {1}"/" --remote "MOD-jbrowses/WormBase/WS$RELEASE/"{1} --AWSACCESS $AWSACCESS --AWSSECRET $AWSSECRET ::: "${SPECIESLIST[@]}"
